...
class TestLegalTransitions:
    def test_draft_to_submitted(
        self, draft_application: application_models.Application
    ) -> None:
    ...
    def test_submitted_to_under_review(
        self, submitted_application: application_models.Application
    ) -> None:
        """Reviewer can start reviewing a submitted application."""
        submitted_application.start_review()
        assert (
            submitted_application.status == application_models.Application.UNDER_REVIEW
        )

    def test_under_review_to_approved(
        self, under_review_application: application_models.Application
    ) -> None:
        """Reviewer can approve an application under review."""
        under_review_application.approve()
        assert (
            under_review_application.status == application_models.Application.APPROVED
        )
    ...
    def test_under_review_to_rejected(
        ...
        assert (
            under_review_application.status == application_models.Application.REJECTED
        )

    def test_under_review_to_draft(
        self, under_review_application: application_models.Application
    ) -> None:
    ...
    def test_return_and_resubmit(
        ...
        assert (
            under_review_application.status == application_models.Application.SUBMITTED
        )
...
class TestIllegalTransitions:
    ...
    def test_cannot_reject_from_draft(
        self, draft_application: application_models.Application
    ) -> None:
        with pytest.raises(django_fsm.TransitionNotAllowed):
            draft_application.reject()

    def test_cannot_start_review_from_draft(
        self, draft_application: application_models.Application
    ) -> None:
        with pytest.raises(django_fsm.TransitionNotAllowed):
            draft_application.start_review()

    def test_cannot_return_for_changes_from_draft(
        self, draft_application: application_models.Application
    ) -> None:
        with pytest.raises(django_fsm.TransitionNotAllowed):
            draft_application.return_for_changes()

    def test_cannot_approve_from_submitted(
        self, submitted_application: application_models.Application
    ) -> None:
        with pytest.raises(django_fsm.TransitionNotAllowed):
            submitted_application.approve()

    def test_cannot_reject_from_submitted(
        self, submitted_application: application_models.Application
    ) -> None:
        with pytest.raises(django_fsm.TransitionNotAllowed):
            submitted_application.reject()

    def test_cannot_return_for_changes_from_submitted(
        self, submitted_application: application_models.Application
    ) -> None:
        with pytest.raises(django_fsm.TransitionNotAllowed):
            submitted_application.return_for_changes()

    def test_cannot_submit_again_from_submitted(
        self, submitted_application: application_models.Application
    ) -> None:
        with pytest.raises(django_fsm.TransitionNotAllowed):
            submitted_application.submit()

    def test_cannot_submit_from_under_review(
        self, under_review_application: application_models.Application
    ) -> None:
        with pytest.raises(django_fsm.TransitionNotAllowed):
            under_review_application.submit()
    ...
    def test_cannot_approve_again_from_approved(
        self, under_review_application: application_models.Application
    ) -> None:
        under_review_application.approve()
        with pytest.raises(django_fsm.TransitionNotAllowed):
    ...
    def test_cannot_submit_from_rejected(
        self, under_review_application: application_models.Application
    ) -> None:
        under_review_application.reject()
        with pytest.raises(django_fsm.TransitionNotAllowed):
...
class TestFSMProtection:
    def test_status_field_is_protected(
        self, draft_application: application_models.Application
    ) -> None:
